...
for /l %%i in (0,1,999) do ( if !RTS_COUNT! geq %%i ( set "num=00%%i" && set "num=!num:~-3!" && ffprobe -v error -select_streams s:!num! -show_entries stream_tags=title -of default=nw=1:nk=1 C\TS.mks > C\N!num! && ffprobe -v error -select_streams s:!num! -show_entries stream_tags=language -of default=nw=1:nk=1 C\TS.mks > C\L!num! && mkvinfo --verbose C\TS.mks > C\TI && if "!num!"=="000" ( set "numshort=0" ) else ( for /f "tokens=* delims=0" %%A in ("!num!") do set "numshort=%%A" )
...
:: Step 27: Cleanup
del C\A && del C\C.mkc && del C\C && del C\IS && del C\IS.mks && del C\S && del C\S.mks && del C\TI && del C\TS && del C\TS.mks && del C\TSF.mks && del C\U && del C\V && del C\VA.mkv && del C\VAC.mkv && for /l %%i in (0,1,999) do (if !RTS_COUNT! geq %%i (set "num=00%%i" && set "num=!num:~-3!" && del "C\D!num!" && del "C\F!num!" && del "C\L!num!" && del "C\N!num!" && del "C\TB!num!" && del "C\TS!num!.ass" && del "C\TSF!num!.mks"))
