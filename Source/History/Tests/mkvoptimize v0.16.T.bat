...
set "CMD=mkvmerge -o C\VA.mkv --no-chapters --no-global-tags --no-track-tags --no-attachments --no-buttons" && set "TRACK_LIST=" && for /f %%A in (C\U) do ( if defined TRACK_LIST ( set "TRACK_LIST=!TRACK_LIST!,1:%%A" ) else ( set "TRACK_LIST=1:%%A" ) )

:: Step 12: Build the subtitle tracklist
type "C\S" | findstr . >nul & if errorlevel 1 ( set /a S_COUNT=0 ) else ( set "CMD2=mkvmerge -o C\S.mks --no-chapters --no-global-tags --no-track-tags --no-attachments --no-buttons --no-video --no-audio" && set "TRACK_LIST2=" && for /f %%A in (C\S) do ( if defined TRACK_LIST2 ( set "TRACK_LIST2=!TRACK_LIST2!,1:%%A" ) else ( set "TRACK_LIST2=1:%%A" ) ) )

:: Step 13: Isolate the video/audio and subtitle tracks
set "CMD=!CMD! --track-order !TRACK_LIST! C\VAC.mkv" && call !CMD! && set VA=C\VA.mkv && set "CMD2=!CMD2! --track-order !TRACK_LIST2! "%INPUT%"" && call !CMD2!

:: Step 14: Detect the imagebased subtitle track IDs from the third output and store the data in a file
...
:: Step 15: Detect the textbased subtitle track IDs from the third output and store the data in a file
...
:: Step 16: Separate the imagebased and textbased subtitle tracks
...
:: Step 17: Conditionally activate the IS variable
if exist C\IS.mks set IS=C\IS.mks

:: Step 18: Detect the number of imagebased subtitle tracks in the fourth output and store that value in a variable
for /f "usebackq tokens=*" %%A in (C\IS) do set /a IS_COUNT+=1

:: Step 19: Detect the number of textbased subtitle tracks in the fifth output and store that value in a variable
for /f "usebackq tokens=*" %%A in (C\TS) do set /a TS_COUNT+=1 && set /a RTS_COUNT=!TS_COUNT! -1

:: Step 20: Extract and optimize the textbased subtitle tracks
...
:: Step 21: Extract the chapter track
mkvextract chapters C\VAC.mkv > C\C && mkvmerge -o C\C.mkc %NULL% --chapters C\C && set C=C\C.mkc

:: Step 22: Detect the name and language flags and store them in file form
...
:: Step 23: Detect the default and forced flags and store them in file form
...
:: Step 24: Store the flags in separate variables
...
:: Step 25: Generate and merge the TSF###.mks files while adding flags to the subtitle tracks and generating a variable
...
:: Step 26: Filter the input name
if "%OUTPUT%"=="C\Input.mkv" ( set OUTPUT=Output.mkv )

:: Step 27: Combine the VA, IS, TSF and C files
