...
for /l %%i in (0,1,999) do (if !RTS_COUNT! geq %%i (set "num=00%%i" && set "num=!num:~-3!" && ffmpeg -y -i C\TS.mks -map 0:s:!num! -f srt C\TS.srt && ffmpeg -y -i C\TS.srt -map 0:s:0 -f ass C\TS.ass && del C\TS.srt && sed -n "/\[Events\]/,$p" C\TS.ass > C\TS.txt && del C\TS.ass && (echo [Script Info] & echo [Styles] & type C\TS.txt) > C\TS!num!.ass && del C\TS.txt ) )
...
for /l %%i in (0,1,999) do (
  if !RTS_COUNT! geq %%i (
    ...
    :: Extract name (title) for stream %%i
    ffprobe -v error -select_streams s:!num! -show_entries stream_tags=title -of default=nw=1:nk=1 C\TS.mks > C\N!num!
    :: Extract language for stream %%i
    ffprobe -v error -select_streams s:!num! -show_entries stream_tags=language -of default=nw=1:nk=1 C\TS.mks > C\L!num!
    
    :: Generate mkvinfo output for current stream
    mkvinfo --verbose C\TS.mks > C\TI!num!
    
    :: Assuming !num! contains something like 001, 002, etc.
    if "!num!"=="000" (
    set "numshort=0"
    ) else (
    for /f "tokens=* delims=0" %%A in ("!num!") do set "numshort=%%A"
    )
    
    :: Reduce the numshort value
    set /a increased_numshort=!numshort! +1
    
    :: Filter the entire TrackInfo file for lines related to this track
    sed -n "/Track number: !increased_numshort!/,+4p" C\TI!num! > C\TB!num!
    
    :: Extract default and forced flags
    sed -n "s/.*Default flag: \([0-9]\+\).*/\1/p" C\TB!num! > C\D!num!
    sed -n "s/.*Forced flag: \([0-9]\+\).*/\1/p" C\TB!num! > C\F!num!
    
    type C\D!num! | findstr . >nul
    if errorlevel 1 (
    (echo 1) > C\D!num!
    )
    
    type C\F!num! | findstr . >nul
    if errorlevel 1 (
    (echo 0) > C\F!num!
    )
  )
...
for /l %%i in (0,1,999) do (if !RTS_COUNT! geq %%i (set "num=00%%i" && set "num=!num:~-3!" && set /p D!num!=<C\D!num! && set /p F!num!=<C\F!num! && set /p L!num!=<C\L!num! && set /p N!num!=<C\N!num!))
...
:: Step 26: Cleanup
del C\A && del C\C.mkc && del C\C && del C\IS && del C\IS.mks && del C\S && del C\S.mks && del C\TS && del C\TS.mks && del C\TSF.mks && del C\U && del C\V && del C\VA.mkv && del C\VAC.mkv && for /l %%i in (0,1,999) do (if !RTS_COUNT! geq %%i (set "num=00%%i" && set "num=!num:~-3!" && del "C\D!num!" && del "C\F!num!" && del "C\L!num!" && del "C\N!num!" && del "C\TS!num!.ass" && del "C\TSF!num!.mks"))
