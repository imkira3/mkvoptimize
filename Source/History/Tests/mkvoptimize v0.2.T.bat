...
:: Step 0: Debug settings and test commands
set INPUT="I.mkv"
set OUTPUT="C\I1.mkv"
...
:: Step 3: Store subtitle stream ID's in a file
ffprobe -v error -select_streams s -show_entries stream=index -of "compact=p=0:nk=1" "I.mkv" > C\S

:: Step 4: Detect the number of subtitle streams and store that value in a variable
for /f "usebackq tokens=*" %%A in ("C\S") do (
    echo %%A
    set /a SUB_COUNT+=1
)
