...
:: Step 11: Build the video and audio tracklist
...
:: Step 12: Build the subtitle tracklist
...
:: Step 13: Isolate the video/audio tracks
...
:: Step 14: Isolate the subtitle tracks
if "%S_COUNT%"=="0" ( set /a S_COUNT=0 ) else ( set "CMD2=!CMD2! --track-order !TRACK_LIST2! "%INPUT%"" && call !CMD2! )

:: Step 15: Detect the imagebased subtitle track IDs from the third output and store the data in a file
...
:: Step 16: Detect the textbased subtitle track IDs from the third output and store the data in a file
...
:: Step 17: Separate the imagebased and textbased subtitle tracks
...
:: Step 18: Conditionally activate the IS variable
if exist C\IS.mks set IS=C\IS.mks

:: Step 19: Detect the number of imagebased subtitle tracks in the fourth output and store that value in a variable
for /f "usebackq tokens=*" %%A in (C\IS) do set /a IS_COUNT+=1

:: Step 20: Detect the number of textbased subtitle tracks in the fifth output and store that value in a variable
for /f "usebackq tokens=*" %%A in (C\TS) do set /a TS_COUNT+=1 && set /a RTS_COUNT=!TS_COUNT! -1

:: Step 21: Extract and optimize the textbased subtitle tracks
...
:: Step 22: Extract the chapter track
mkvextract chapters "%INPUT%" > C\C && mkvmerge -o C\C.mkc %NULL% --chapters C\C && set C=C\C.mkc

:: Step 23: Detect the name and language flags and store them in file form
...
:: Step 24: Detect the default and forced flags and store them in file form
...
:: Step 25: Store the flags in separate variables
...
:: Step 26: Generate and merge the TSF###.mks files while adding flags to the subtitle tracks and generating a variable
...
:: Step 27: Filter the input name
if "%OUTPUT%"=="C\Input.mkv" ( set OUTPUT=Output.mkv )

:: Step 28: Combine the VA, IS, TSF and C files
