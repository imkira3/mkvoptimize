...
set "CMD=mkvmerge -o C\VA.mkv --no-chapters --no-global-tags --no-track-tags --no-attachments --no-buttons" && set "TRACK_LIST=" && for /f %%A in (C\U) do ( if defined TRACK_LIST ( set "TRACK_LIST=!TRACK_LIST!,1:%%A" ) else ( set "TRACK_LIST=1:%%A" ) ) && set "CMD=!CMD! --track-order !TRACK_LIST!" && set "CMD=!CMD! C\VAC.mkv" && call !CMD! && set VA=C\VA.mkv
...
:: Step 16: Conditionally activate the IS variable
